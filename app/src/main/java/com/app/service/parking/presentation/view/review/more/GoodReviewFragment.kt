package com.app.service.parking.presentation.view.review.more
        ...
        with(binding) {
            ...
            moreReviewRVAdapter = MoreReviewRVAdapter(object: RecyclerItemClickListener {
                ...
                override fun onClick(position: Int, resId: Int?) {
                    ...
                    val imgUri = viewModel?.goodReviewList?.value?.get(position)?.reviewImageUrl?.replace("https", "http")
                    // 이미지 팝업화면을 보여준다.
            ...
            // 리뷰 데이터를 실시간 관찰
            viewModel?.goodReviewList?.observe(this@GoodReviewFragment) { reviewList ->
                // 리사이클러뷰 데이터 갱신
